# RIQSIY Regional 2.0

- [x] Fase 0 — Proteger la base actual y normalizar el nombre visible “Machu Moqo” sin romper rutas guardadas; mantener **RIQSIY** como nombre principal.
- [x] Fase 1 — Consolidar Machu Moqo, evidencia de campo y aventura educativa central.
- [x] Fase 2 — Mejorar la exploración 3D y sus descubrimientos educativos.
- [x] Fase 3 — Crear RIQSIY Yachay en quechua Cusco-Collao con contenido previamente validado.
- [x] Fase 4 — Integrar Pasaporte RIQSIY y ampliar progreso, sellos, insignias y racha.
- [x] Fase 5 — Crear Detective del Patrimonio y los laboratorios de construcción y agua.
- [x] Fase 6 — Crear Voces de mi comunidad y Descubre patrimonio cerca de ti con privacidad y lenguaje prudente.
- [x] Fase 7 — Crear Yachaq RIQSIY con clasificación explícita de la evidencia.
- [x] Fase 8 — Consolidar Laboratorio RIQSIY y Modo Jurado sin inventar resultados.
- [x] Fase 9 — Mejorar Antes y Ahora, Mi compromiso, accesibilidad y revisión multidispositivo.

## Mejoras solicitadas

- [x] Convertir RIQSIY Yachay en una experiencia de lecciones gamificadas con identidad andina.
- [x] Igualar PIRQA a la aplicación de referencia autorizada y verificar ambas experiencias.
- [x] Ampliar Descubre y el mapa con aproximadamente 20 sitios reales, fotografías públicas acreditadas y fichas prudentes.
- [x] Ajustar Investigación, Modo Jurado, FENCYT y Detective del Patrimonio al prompt maestro regional.
- [x] Verificar rutas prioritarias y experiencia móvil sin modificar la arquitectura principal.
- [x] Completar el pulido definitivo FENCYT: evidencia del proceso, 8 niveles Yachay, Pasaporte, modo feria, resultados vacíos, reflexión y relato de origen.
