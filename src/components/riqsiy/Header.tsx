...
export function Header() {
  ...
  const { puntos, hidratado } = useProgreso();
    ...
    <header className="sticky top-0 z-50 border-b border-border/70 bg-background/85 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center gap-3 px-4 py-3">
        <Link to="/" className="flex items-center gap-2" onClick={() => setAbierto(false)}>
          ...
          <span className="font-display text-lg font-semibold tracking-[0.18em] text-primary">RIQSIY</span>
        </Link>
        ...
        <span className="ml-auto rounded-full border border-accent/50 bg-accent/15 px-3 py-1 text-xs font-semibold text-foreground lg:ml-3">
          ✦ {hidratado ? puntos : 0} pts
        </span>
