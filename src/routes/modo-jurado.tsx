...
function Jurado() {
  ...
  return <div className={`surface-deep stone-grid flex min-h-[75vh] flex-col px-4 py-6 ${exposicion ? "fixed inset-0 z-[100] min-h-screen" : ""}`}>
    <div className="mx-auto flex w-full max-w-5xl flex-1 flex-col">
      <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-xs font-semibold text-gold-soft">MODO FERIA · {String(i + 1).padStart(2, "0")} / {P.length}</p><div className="flex gap-2"><Button asChild size="sm" variant="outline"><Link to="/fencyt"><Home />Volver al modo feria</Link></Button><Button size="sm" variant="outline" onClick={() => setExposicion((v) => !v)}>{exposicion ? <Minimize2 /> : <Mic2 />}{exposicion ? "Salir" : "Exponer"}</Button></div></div>
      ...
      <div className="grid grid-cols-2 gap-3"><Button size="lg" variant="outline" disabled={i === 0} onClick={() => setI((v) => v - 1)}><ArrowLeft />Anterior</Button>{i < P.length - 1 ? <Button size="lg" onClick={() => setI((v) => v + 1)}>Siguiente<ArrowRight /></Button> : <Button size="lg" asChild><Link to="/fencyt">Cerrar<ArrowRight /></Link></Button>}</div>
    </div>
  </div>;
}