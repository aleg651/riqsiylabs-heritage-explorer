...
function Detective() {
  ...
  return <main className="mx-auto max-w-5xl px-4 py-14">
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
      <figure className="overflow-hidden rounded-lg border border-border bg-card"><img src={REGISTROS_CAMPO[nivel % REGISTROS_CAMPO.length]?.src} alt="Registro propio usado como evidencia del reto" className="aspect-[4/3] h-full w-full object-cover" /><figcaption className="grid gap-2 p-4 text-xs text-muted-foreground"><span><strong className="text-foreground">Ubicación general:</strong> Machu Moqo, entorno de la comunidad.</span><span><strong className="text-foreground">Pista:</strong> Describe solamente aquello que la imagen permite comprobar.</span><span><strong className="text-foreground">Observación registrada:</strong> {REGISTROS_CAMPO[nivel % REGISTROS_CAMPO.length]?.observacion}</span></figcaption></figure>
      <section className="rounded-lg border border-border bg-card p-5 sm:p-6">
        ...
        {revisado && <div className={`mt-5 rounded-md border p-4 text-sm ${correcto ? "border-jade bg-jade/10" : "border-accent bg-accent/10"}`}><p className="font-semibold">{correcto ? "Conclusión respaldada" : "Vuelve a revisar la evidencia"}</p><p className="mt-1 text-muted-foreground">{actual.explicacion}</p>{correcto && <p className="mt-3 border-t border-current/20 pt-3 text-xs"><strong>¿Qué falta investigar?</strong> {REGISTROS_CAMPO[nivel % REGISTROS_CAMPO.length]?.pregunta}</p>}{correcto && <Button className="mt-4 w-full" onClick={continuar}>{terminado ? "Registrar caso resuelto" : "Siguiente nivel"}</Button>}</div>}
      </section>
    </div>
